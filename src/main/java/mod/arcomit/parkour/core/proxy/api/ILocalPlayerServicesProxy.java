 ...
 * 本地玩家专属服务代理接口。
 * <p>
 * 封装仅在客户端可用的 {@code LocalPlayer} 方法——位置同步、移动状态、
 * 饥饿度检查等。状态机通过此接口获取这些信息，避免在服务端触发
...
public interface ILocalPlayerServicesProxy {
	 ...
	 *
	 * @param player 目标玩家，不可为null
	 * @return 饱食度充足时返回 {@code true}；饱食度过低或服务端返回 {@code false}
	 */
	boolean hasEnoughFoodToStartSprinting(Player player);
}
