...
public class CameraAnimationRegistry implements ResourceManagerReloadListener {
	...
	private final Map<Identifier, CameraAnimation> animations = new HashMap<>();
		...
		for (Map.Entry<Identifier, Resource> entry : resources.entrySet()) {
			...
			try (BufferedReader reader = entry.getValue().openAsReader()) {
				...
				for (String animationName : animationsObject.keySet()) {
					...
					Identifier animationId = Identifier.fromNamespaceAndPath(
							fileLocation.getNamespace(), validPath);
	 ...
	 */
	public CameraAnimation getAnimation(Identifier id) {
