 ...
 * <p>
 * 沿指定水平方向生成两个 AABB 检测盒：一个位于玩家头部高度（85%~100% 身高），
 * 一个位于脚部高度（0%~30% 身高）。只有当<b>两个检测盒同时</b>与方块碰撞时，
 * 才认为该方向满足贴墙条件。
 * </p>
 * <p>
 * 头部+脚部同时碰撞保证了玩家前方是一面完整墙面而非半高的台阶或栅栏。
 * 忽略的方块类型由标签 {@code COMMON_IGNORED_BLOCKS} 定义。
 * </p>
 *
 * @author Mitok
 * @since 2026-06-08
