 ...
 * {@link SwimData}、{@link InputData}、{@link StateData}。
 *
 ...
 * @since 2026-06-08
 */
public record ParkourContext(GroundMovementData ground,
                             WallMovementData wall, JumpData jump,
                             SwimData swim, InputData input, StateData state) {

	public static final Codec<ParkourContext> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
							GroundMovementData.CODEC.optionalFieldOf("ground", new GroundMovementData())
									.forGetter(ParkourContext::ground),
							WallMovementData.CODEC.optionalFieldOf("wall", new WallMovementData())
									.forGetter(ParkourContext::wall),
							JumpData.CODEC.optionalFieldOf("jump", new JumpData())
									.forGetter(ParkourContext::jump),
							SwimData.CODEC.optionalFieldOf("swim", new SwimData())
	...
	public static final StreamCodec<ByteBuf, ParkourContext> STREAM_CODEC =
			StreamCodec.composite(
					GroundMovementData.STREAM_CODEC, ParkourContext::ground,
					WallMovementData.STREAM_CODEC, ParkourContext::wall,
					JumpData.STREAM_CODEC, ParkourContext::jump,
					SwimData.STREAM_CODEC, ParkourContext::swim,
					InputData.STREAM_CODEC, ParkourContext::input,
					StateData.STREAM_CODEC, ParkourContext::state,
					ParkourContext::new);
	...
	public ParkourContext() {
		this(new GroundMovementData(), new WallMovementData(), new JumpData(),
				new SwimData(), new InputData(), new StateData());
	}
