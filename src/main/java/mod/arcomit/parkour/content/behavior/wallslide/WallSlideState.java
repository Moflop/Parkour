 ...
 * 水平速度乘以 0.8、垂直速度乘以 0.7 逐步减速，同时吸附在墙面上。
 * 每 tick 自动检测并切换最近的墙体方向，方向变化时通过 S2C 包同步到其他客户端。
 * 与跑墙/爬墙不同，滑墙不要求玩家冲刺。
 *
 * @author Mitok
 * @since 2026-06-08
...
public class WallSlideState extends AbstractParkourState {
    ...
    /**
     * 构造并注册状态转换：每客户端 tick 检测跳跃释放宽限期是否结束，
     * 宽限期过后自动退回到默认状态。
     */
     ...
     * @return 配置启用、不在地面、不在可攀爬方块、不在水/熔岩中、
     *         可执行跑酷行为、且附近存在有效墙体时返回 true
     */
