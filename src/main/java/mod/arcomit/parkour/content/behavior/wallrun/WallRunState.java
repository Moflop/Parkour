 ...
 * 运行时玩家以配置的速度沿面朝方向移动，服务端每 tick 播对应墙面材质脚步声。
 * 停止前进输入或宽限期结束后自动退出，退出时可垂直向上跳跃（墙跳）。
 *
 * @author Mitok
 * @since 2026-06-08
 */
public class WallRunState extends AbstractParkourState {
    ...
    /**
     * 构造并注册状态转换：每客户端 tick 检测前进输入是否归零或跳跃释放宽限期是否结束，
     * 满足任一即切回默认状态。
     */
    public WallRunState() {
        registerTransitions(
     ...
     * @return 配置启用、冲刺中、不在地面、不在可攀爬方块、不在水/熔岩中、
     *         可执行跑酷行为、且前方无阻挡时返回 true
     */
    public static boolean meetsBaseConditions(Player player) {
		...
		// 确保正前方没有撞墙
		if (BlockCollisions.isBlockedTowards(player,
				player.getDirection(), ParkourTags.Blocks.COMMON_IGNORED_BLOCKS)) {
			return false;
		}
		return true;
	}
    ...
    /**
     * 进入跑墙时初始化物理运动方向、锁定碰撞墙体方向，并重置平行跳跃记录
     * 使玩家跑墙后可向运动方向再次跳跃。
     */
    @Override
     ...
     * 且玩家面朝方向与记录的运动方向偏差不超过
     * {@value #FACING_ANGLE_TOLERANCE} 度。
     *
     * @param player  目标玩家，不能为 null
     * @param context 跑酷上下文，不能为 null
