 ...
 * <p>
 * 当玩家水平移动到墙角尽头、失去当前面悬挂点时，先沿原方向微推一小段
 * 使玩家接触到侧面，然后检测侧面是否可悬挂。若能则切换方向并同步服务端，
 * 若不能则将玩家位置回退到移动前。
 *
 * @author Mitok
 * @since 2026-06-08
...
public class ClientArmhangRotation {
	...
	/**
	 * 尝试外角旋转：微推玩家使其接触相邻墙面，检测并切换悬挂方向。
	 * 若切换成功则向服务端同步新的悬挂方向和位置；若失败则回退位置。
	 *
	...
	public static void tryOutsideCornerRotation(Player player, float leftImpulse,
		...
		if (ArmhangCollision.hasValidHangPoint(player, newDirection)) {
			...
			PacketDistributor.sendToServer(new SyncArmhangDirC2SPayload(newDirection));
		} else {
