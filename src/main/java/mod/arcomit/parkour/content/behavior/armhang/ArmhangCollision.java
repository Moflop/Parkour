 ...
 * 手臂悬挂的碰撞检测工具。
 * <p>
 * 悬挂需要两个条件同时满足：脚底有可站的方块支撑盒（通过 ArmhangTopSensor 检测），
 * 头部有可抓的墙壁（通过 ArmhangEyeSensor 检测）。此外还要排除地面高度过近的情况，
 * 防止玩家在平地栏杆上误触发悬挂。
 *
 * @author Mitok
 * @since 2026-06-08
...
public class ArmhangCollision {
	 ...
	 * 脚底离地阈值（单位：格）。
	 * <p>
	 * 悬挂时头部以上 1.02 格有碰撞，栅栏高度 1.5，1.5 - 1.02 = 0.48，
	 * 因此玩家站在地面上时不会被误判为可悬挂。
	 */
	...
	public static boolean isTooCloseToGround(Player player) {
		...
		AABB checkBox = new AABB(feetBox.minX, feetBox.minY - GROUND_CLEARANCE, feetBox.minZ,
				feetBox.maxX, feetBox.minY, feetBox.maxZ);
	 ...
	 * <p>
	 * 采用双传感器逻辑：眼部传感器（检测头部可抓墙面）和顶部传感器（检测脚底有支撑盒），
	 * 任一传感器返回 true 即认为可以悬挂。这样能兼容不同方块形状（如栅栏、台阶边缘）。
	 *
	 * @param player    目标玩家，不可为 null
