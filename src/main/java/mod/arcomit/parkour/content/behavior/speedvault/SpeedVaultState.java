 ...
 * 进入后持续 2 tick 即自动退出，退出时给予短暂的速度提升效果。
 * 按住潜行键可提前退回到默认状态。
 *
 * @author Mitok
 * @since 2026-06-08
...
public class SpeedVaultState extends AbstractParkourState {
     ...
     * @return 所有基础条件满足返回 true，以下任一情况返回 false：
     *         配置禁用、玩家在攀爬梯子/藤蔓、潜行中、在水中或熔岩中、或不可执行跑酷行为
     */
    ...
    /**
     * 客户端进入速过状态时触发：播放翻越音效并随机选择左手/右手动画，
     * 同时将当前坐标发送给服务端以校准位置。
     */
    @Override
    ...
    /**
     * 每模拟 tick 根据障碍物高度计算垂直速度、根据 WASD 输入计算水平速度，
     * 直接设置玩家速度实现翻越位移。
     */
    @Override
    ...
    /**
     * 退出速过状态时，根据玩家当前移动输入赋予一个向前的水平初速度，
     * 垂直速度交由重力接管。
     */
    @Override
     ...
     * 障碍物上方有通行空间、玩家在地面或非快速下落状态。
     * 通过后会将障碍物高度记录到 {@code context.wall()} 中供物理计算使用。
     *
     * @param player 目标玩家，不能为 null
