 ...
 * 持续期间服务器每 tick 播放对应墙面材质的脚步声。
 * 松开跳跃键或停止前进输入即退出，退出时速度归零以避免手感卡顿。
 *
 * @author Mitok
 * @since 2026-06-08
 */
public class WallClimbState extends AbstractParkourState {
    ...
    /**
     * 构造并注册状态转换：每客户端 tick 检测前进输入是否归零或跳跃键是否松开，
     * 满足任一条件则切回默认状态。
     */
    ...
    /**
     * 进入爬墙状态时记录当前面朝方向作为攀爬方向，同时重置上跳方向记录，
     * 使玩家在爬墙后可以向反方向再次跳跃（爬墙刷新跳跃次数）。
     */
    @Override
