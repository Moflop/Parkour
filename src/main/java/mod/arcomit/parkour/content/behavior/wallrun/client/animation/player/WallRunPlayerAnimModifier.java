...
public class WallRunPlayerAnimModifier extends AbstractModifier {
	...
	public void get3DTransform(@NotNull PlayerAnimBone bone) {
		...
		if (boneName.equals("head")) {
			bone.rotation.x = WallRunAnimMath.rotLerpRadians(bone.rotation.x,
					this.currentFrameData.headRotX);
			bone.rotation.y = WallRunAnimMath.rotLerpRadians(bone.rotation.y,
					this.currentFrameData.headRotY);
			bone.rotation.z = WallRunAnimMath.rotLerpRadians(bone.rotation.z,
		...
		if (this.currentFrameData.hasMotion) {
			switch (boneName) {
				case "left_leg":
					bone.rotation.x += this.currentFrameData.leftLegRotX;
					break;
				case "right_leg":
					bone.rotation.x += this.currentFrameData.rightLegRotX;
					break;
				case "left_arm":
				...
				case "right_arm":
					...
					break;
				case "torso":
					bone.rotation.y += this.currentFrameData.torsoRotY;
					break;
				case "body":
