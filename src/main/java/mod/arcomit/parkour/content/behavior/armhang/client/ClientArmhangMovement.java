 ...
 * 客户端悬挂平移处理。
 * <p>
 * 仅当玩家面向悬挂方向（误差 75 度以内）且有横向输入时才执行移动。
 * 移动后若悬挂点依然有效则结束；否则委托给外角旋转处理。
 *
 * @author Mitok
 * @since 2026-06-08
 */
public class ClientArmhangMovement {
	 ...
	 * 根据玩家输入执行悬挂状态下的水平移动。
	 * <p>
	 * 移动方向由悬挂方向的法向量和输入的左右符号共同决定，
	 * 速度由 {@code ParkourConfig.armhangMoveSpeed} 控制。
	...
	/**
	 * 根据输入方向和悬挂方向计算水平移动向量。
	 * 速度大小为配置的固定值，方向为悬挂法线方向（左右）乘以输入符号。
	 *
