...
public class BlockCollisions {
	...
	/**
	 * 根据实体朝向和高度比例构造一个用于方向性碰撞检测的包围盒。 包围盒沿指定方向扩展 {@code distance} 格，高度范围由实体身高比例决定， 默认收缩 0.001
	 * 以避免与自身脚下方块误判。
	 *
	 ...
	 */
	public static AABB buildBox(LivingEntity entity, Direction direction, double minHeightRatio,
			double maxHeightRatio, double distance) {
		Vec3 pos = entity.position();
		double halfWidth = entity.getBbWidth() / 2;
		double height = entity.getBbHeight();

		AABB base = new AABB(pos.x - halfWidth, pos.y + height * minHeightRatio,
				pos.z - halfWidth, pos.x + halfWidth,
				pos.y + height * maxHeightRatio, pos.z + halfWidth).deflate(
	 ...
	 */
	public static boolean isBlockedTowards(LivingEntity entity, Direction direction,
	 ...
	 */
	public static boolean isBlockedTowards(LivingEntity entity, Direction direction,
			double range, TagKey<Block> skipTag) {
	...
	/**
	 * 判断指定包围盒在关卡中是否与任何符合条件的方块发生碰撞。 会跳过 skipTag 标记的方块以及空气/非固体方块。
	 *
	 ...
	 */
	public static boolean hasBlockCollision(Level level, AABB box, TagKey<Block> skipTag) {
	...
	private static Iterable<VoxelShape> filterBlocks(Level level, AABB box,
			TagKey<Block> skipTag) {
		return () -> new FilteredBlockCollisions<>(level, null, box, false, skipTag,
				(pos, shape) -> shape);
	}
}
